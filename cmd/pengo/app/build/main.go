...
func main() {
    ...
    router.GET("/login/:userid/:password", func(response http.ResponseWriter, request *http.Request, params httprouter.Params) {
    		c := App {
    		...
    		c.InitAction()
    		c.BeforeAction(c)
    		c.Action(c, `{"Order":["userid","password"],"Type":{"password":"string","userid":"int"}}`, params)
    		c.AfterAction(c)
    		c.Flow.Graph()
		})
